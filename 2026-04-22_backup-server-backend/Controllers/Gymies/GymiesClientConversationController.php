...
final class GymiesClientConversationController extends Controller
{
    ...
    public function messages(Request $request, string $conversationId): JsonResponse
    {
        ...
        // N-017 FIXED: IDOR voorkomen — user moet participant zijn
        $conversation = DB::table('gymies_conversations')
            ->where('id', $conversationId)
            ->where('client_user_id', (int) $user->id)
            ->first();
        if (!$conversation) {
            // Controleer extra of user participant is (defense-in-depth)
            $isParticipant = DB::table('gymies_conversation_participants')
                ->where('conversation_id', $conversationId)
                ->where('user_id', (int) $user->id)
                ->exists();
            if (!$isParticipant) {
                return response()->json(['message' => 'Geen toegang tot dit gesprek.'], 403);
            }
