...
final class GymiesSeoController extends Controller
{
    ...
    public function sitemap(): Response
    {
        ...
        // Trainer-profielen — nooit een crash bij DB-fout of ontbrekende tabel
        // N-021 FIXED: gevoelige trainer-data verwijderd uit publieke SEO response
        try {
            if (Schema::hasTable('gymies_users') && Schema::hasTable('gymies_trainer_profiles')) {
                $trainers = DB::table('gymies_users as u')
                    ...
                    ->where(function ($q) {
                        $q->whereNull('p.moderation_status')
                          ->orWhere('p.moderation_status', 'approved');
                    })
                ...
                foreach ($trainers as $t) {
                    // URL-formaat overeenkomstig Next.js /trainers/[id] pagina
                    $slug = (int) $t->id;
                    if (!empty($t->display_name)) {
                        $slug = strtolower(preg_replace('/[^a-z0-9]+/i', '-', trim($t->display_name))) . '-' . (int) $t->id;
                    }
                    $loc = self::BASE_URL . '/trainers/' . $slug;
                    $xml .= "  <url>\n";
                    $xml .= "    <loc>" . htmlspecialchars($loc, ENT_XML1) . "</loc>\n";
                    $xml .= "    <lastmod>{$lastmod}</lastmod>\n";
            ...
            // DB niet bereikbaar of andere fout — sitemap wordt zonder trainer-URLs geretourneerd
        }
