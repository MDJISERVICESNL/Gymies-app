...
import '../screens/trainer_income_screen.dart';
...
class DeepLinkService {
  ...
  static String buddyInviteUrl({
    ...
    if (trainerName != null && trainerName.isNotEmpty) {
      params['trainer'] = Uri.encodeComponent(trainerName);
    }
    final q = params.entries.map((e) => '${e.key}=${e.value}').join('&');
    return '${baseUrl}buddy/join?$q';
  }
  ...
  static Widget? screenFromUri(Uri uri) {
    ...
    // Payment return (bestaand)
    if (host == 'payment' && path.isEmpty) {
    ...
    if (host == 't' || (host.isEmpty && path.startsWith('t/'))) {
      ...
      final slug = segments.length >= 2 ? segments[1] : (segments.isNotEmpty ? segments.first : null);
    ...
    if (host == 'trainer') {
      switch (path) {
        ...
        case 'income':
          return const TrainerIncomeScreen();
        case 'messages':
