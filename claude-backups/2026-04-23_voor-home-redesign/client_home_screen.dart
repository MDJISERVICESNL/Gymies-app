...
class _ClientHomeScreenState extends State<ClientHomeScreen>
  ...
  String? _statsError;
  Set<int> _weekSessionDays = {}; // 0=Ma, 1=Di, ..., 6=Zo — echte boekingsdagen
  ...
  Future<void> _loadStats() async {
    try {
      ...
      // Alle komende + afgeronde boekingen deze week → dots op de kalender
      final weekDays = <int>{};
      for (final b in bookings) {
        final d = b.scheduledAt;
        if (!d.isBefore(thisWeekStart) && d.isBefore(thisWeekEnd)) {
          weekDays.add(d.weekday - 1); // 0=Ma … 6=Zo
        }
      }

      final completedBookings = bookings.where((b) =>
        b.status == 'completed' || b.status == 'confirmed' ||
        b.status == 'checked_in' || b.status == 'done').toList();
      final thisWeekCompleted = completedBookings.where((b) {
        final d = b.scheduledAt;
      ...
      debugPrint('[ClientHome] Stats laden mislukt: $e');
      if (mounted) {
        setState(() {
          _weekSessionDays = {};
          _stats = null; // null = laden mislukt, toon dash-waarden
          _loadingStats = false;
        });
      }
    }
  ...
  Widget _buildWeekDays() {
    ...
    // Bereken de echte datums van deze week (maandag t/m zondag)
    ...
    return Row(
      ...
      children: List.generate(7, (index) {
        ...
        final hasSession = _weekSessionDays.contains(index);
  ...
  Widget _buildStatsRow() {
    final streakCount = _stats?['streak'] ?? 0;
    final thisWeek = _stats?['this_week'] ?? 0;
    final totalSessions = _stats?['total'] ?? 0;
  ...
  Widget _buildAchievementCard() {
    final achievementsUnlocked = _stats?['achievements_unlocked'] ?? 0;
    final totalAchievements = _stats?['total_achievements'] ?? 0;
