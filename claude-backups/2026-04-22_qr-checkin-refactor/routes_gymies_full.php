...
Route::prefix('api/gymies')->name('api.gymies.')->group(function () {
    ...
    Route::get('debug-auth', function (\Illuminate\Http\Request $r) {
        $authHeader = $r->header('Authorization') ?? $r->server('HTTP_AUTHORIZATION');
        $xToken = $r->header('X-Gymies-Access-Token') ?? $r->server('HTTP_X_GYMIES_ACCESS_TOKEN');
        $h = $authHeader ?? ($xToken ? 'Bearer ' . $xToken : null) ?? $r->header('X-Authorization') ?? $r->header('X-Gymies-Token');
        $has = $h !== null && $h !== '';
        $bearer = $has && str_starts_with($h, 'Bearer ');
        $tokenValue = $bearer ? trim(substr($h, 7)) : ($xToken ? trim($xToken) : null);
        $queryToken = $r->query('access_token');
        $hasQuery = $queryToken !== null && trim((string) $queryToken) !== '';
        return response()->json([
            'auth_header_received' => $has,
            'bearer_prefix' => $bearer,
            'x_gymies_token_received' => $xToken !== null && trim((string) $xToken) !== '',
            'token_length' => $tokenValue ? strlen($tokenValue) : 0,
            'access_token_in_query' => $hasQuery,
            'query_token_length' => $hasQuery ? strlen(trim((string) $queryToken)) : 0,
        ]);
    })->name('debug.auth');

    // Debug: controleer of ontvangen token in DB staat (zelfde volgorde als middleware: query eerst)
    // Controleert: gymies_sessions (64-hex) + Sanctum-tabellen (hashed)
    Route::get('debug-token-check', function (\Illuminate\Http\Request $r) {
        $token = null;
        if ($r->filled('access_token')) {
            $token = trim(rawurldecode((string) $r->input('access_token')));
        }
        if ($token === null || $token === '') {
            $h = $r->header('Authorization');
            if (is_string($h) && str_starts_with($h, 'Bearer ')) {
                $token = trim(substr($h, 7));
            } elseif ($r->header('X-Gymies-Access-Token')) {
                $token = trim((string) $r->header('X-Gymies-Access-Token'));
            } else {
                $raw = $r->server('HTTP_AUTHORIZATION') ?? $r->server('HTTP_X_GYMIES_ACCESS_TOKEN');
        ...
        $foundSanctum = false;
        $foundGymiesSessions = false;
        $gymiesSessionsUserId = null;
        if ($token !== '') {
            $plain = str_contains($token, '|') ? substr($token, strpos($token, '|') + 1) : $token;
            $hash = hash('sha256', $plain);
            foreach (['gymies_personal_access_tokens', 'personal_access_tokens'] as $tbl) {
                if (\Illuminate\Support\Facades\Schema::hasTable($tbl)) {
                    $row = \Illuminate\Support\Facades\DB::table($tbl)->where('token', $hash)->first();
                    if ($row !== null) {
                        $foundSanctum = true;
                        break;
                    }
            ...
            // 64-hex: zoek in gymies_sessions (zelfde logica als middleware)
            if (strlen($plain) === 64 && ctype_xdigit($plain) && \Illuminate\Support\Facades\Schema::hasTable('gymies_sessions')) {
                $q = \Illuminate\Support\Facades\DB::table('gymies_sessions')->where('token', $plain);
                if (\Illuminate\Support\Facades\Schema::hasColumn('gymies_sessions', 'expires_at')) {
                    $q->where(function ($sq) {
                        $sq->whereNull('expires_at')->orWhere('expires_at', '>', now());
                    });
                }
                if (\Illuminate\Support\Facades\Schema::hasColumn('gymies_sessions', 'revoked_at')) {
                    $q->whereNull('revoked_at');
                }
                $row = $q->first();
                if ($row !== null && isset($row->user_id)) {
                    $foundGymiesSessions = true;
                    $gymiesSessionsUserId = (int) $row->user_id;
                }
            }
        }
        return response()->json([
            ...
            'token_has_pipe' => $token !== '' && str_contains($token, '|'),
            'token_preview' => strlen($token ?? '') > 8 ? substr($token, 0, 8) . '...' . substr($token, -4) : '***',
            'token_is_64hex' => $token !== '' && strlen(preg_replace('/[^a-fA-F0-9]/', '', $token)) === 64,
            'found_in_sanctum' => $foundSanctum,
            'found_in_gymies_sessions' => $foundGymiesSessions,
            'gymies_sessions_user_id' => $gymiesSessionsUserId,
            'found_in_db' => $foundSanctum || $foundGymiesSessions,
        ]);
    ...
    Route::get('trainers/by-slug/{slug}', [\App\Http\Controllers\Gymies\GymiesTrainerSlugController::class, 'show'])->name('trainers.by-slug');
    ...
    Route::get('trainers/{id}/reviews', [\App\Http\Controllers\Gymies\GymiesTrainerReviewsController::class, 'index'])->name('trainers.reviews');
    ...
    Route::match(['get', 'post'], 'cron/expire-subscription-trials', \App\Http\Controllers\Gymies\ExpireSubscriptionTrialsController::class)->name('cron.expire-subscription-trials');
    ...
    Route::middleware([
        ...
        Route::get('trainer/documents', [\App\Http\Controllers\Gymies\GymiesTrainerDocumentsController::class, 'index'])->name('trainer.documents.index');
        Route::put('trainer/documents', [\App\Http\Controllers\Gymies\GymiesTrainerDocumentsController::class, 'update'])->name('trainer.documents.update');
        Route::patch('trainer/documents', [\App\Http\Controllers\Gymies\GymiesTrainerDocumentsController::class, 'update'])->name('trainer.documents.patch');
        Route::post('trainer/documents', [\App\Http\Controllers\Gymies\GymiesTrainerDocumentsController::class, 'update'])->name('trainer.documents.update.post');
        ...
        Route::middleware('gymies.idempotency')->group(function () {
            ...
            Route::post('bookings/{id}/confirm-cash', [\App\Http\Controllers\Gymies\GymiesBookingController::class, 'confirmCashPayment'])->name('bookings.confirm-cash');
        ...
        Route::get('trainer/clients/{clientUserId}/payments', [\App\Http\Controllers\Gymies\GymiesTrainerClientPaymentsController::class, 'index'])->name('trainer.clients.payments.index');
        ...
        Route::post('subscription/change', \App\Http\Controllers\Gymies\ChangeSubscriptionController::class)->name('subscription.change');
        ...
        Route::prefix(trim((string) (env('GYMIES_ADMIN_PREFIX') ?? env('TRAINMAAT_ADMIN_PREFIX', 'vault-console'))) ?: 'vault-console')
            ...
            ->group(function () {
                ...
                Route::post('users/{userId}/tier', [\App\Http\Controllers\Gymies\GymiesAdminController::class, 'updateUserTier'])
                    ->middleware(['gymies.idempotency', 'gymies.admin.capability:admin.users.manage'])
                    ->name('users.tier');
                Route::post('users/{userId}/assign-subscription', [\App\Http\Controllers\Gymies\AssignSubscriptionController::class, 'assign'])
                    ->middleware(['gymies.idempotency', 'gymies.admin.capability:admin.users.manage'])
