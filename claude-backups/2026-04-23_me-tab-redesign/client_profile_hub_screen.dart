...
class _ProfilTabState extends State<_ProfilTab>
  ...
  List<Widget> _buildStaggeredItems() {
    final items = [
      ...
      _SettingsTile(
        ...
        title: 'Noodcontact instellen',
        subtitle: 'Voor SOS-alerts tijdens safe sessions',
        onTap: () => widget.onPush(const ClientProfileScreen()),
      ),
      const SizedBox(height: 16),
      ...
      children: [
        ...
        Opacity(
          ...
          child: _QuickActionButton(
            ...
            label: 'Achievements',
            color: Colors.grey,
            onTap: () => _showSnackBar('Achievements komen binnenkort!'),
          ),
        ),
        _QuickActionButton(
          ...
          onTap: () => widget.onPush(const ReferralScreen()),
        ),
        _QuickActionButton(
          icon: Icons.headset,
          label: 'Support',
  ...
  /// Bouw een persoonlijke referral link met user ID voor conversie-tracking.
  /// Format: https://www.gymies.nl/?ref=<user_id>
  String _referralLink() {
    final user = context.read<AuthService>().user;
    final userId = (user?['id'] ?? user?['user_id'] ?? '').toString().trim();
    if (userId.isNotEmpty) {
      return '${AppConfig.shareBaseUrl}/?ref=$userId';
    }
    return AppConfig.shareBaseUrl;
  }

  String _referralShareText() {
    final link = _referralLink();
    return '${AppConfig.shareText}\n\n$link';
  }

  void _showShareSheet() {
    final shareText = _referralShareText();
      ...
      builder: (ctx) => Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              'Deel je persoonlijke link en help vrienden fitnessdoelen bereiken',
            ...
            Row(
              ...
              children: [
                _ShareOption(
                  ...
                  onTap: () {
                    Navigator.pop(ctx);
                    Share.share(shareText, subject: AppConfig.shareSubject);
                  },
                ),
                _ShareOption(
                ...
                _ShareOption(
                  ...
                  onTap: () {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
                        content: Text('Referral link gekopieerd!'),
  ...
  List<Widget> _buildStaggeredItems() {
    final items = [
      ...
      _SettingsTile(
        ...
        subtitle: _calendarSyncEnabled
            ? 'Sessies worden automatisch aan je agenda toegevoegd'
            : 'Koppel sessies met Apple of Google Calendar',
        trailing: Switch.adaptive(
          value: _calendarSyncEnabled,
          activeColor: GymiesColors.primary,
          onChanged: (value) async {
            setState(() => _calendarSyncEnabled = value);
            await CalendarService.instance.setAutoSync(value);
            if (mounted) {
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                  content: Text(
                    value
                        ? 'Nieuwe sessies worden automatisch aan je agenda toegevoegd'
