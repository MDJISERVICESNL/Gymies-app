...
class ClientShellState extends State<ClientShell> {
  ...
  Future<void> _checkOnboarding() async {
    try {
      final done = await isClientOnboardingDone();
      if (done) {
        // Onboarding al voltooid/overgeslagen
        if (mounted) {
          setState(() {
      ...
      if (user != null) {
        ...
        if (name.isNotEmpty && city.isNotEmpty) {
          // Bestaande gebruiker met compleet profiel — skip onboarding
          await markClientOnboardingDone();
          if (mounted) {
            setState(() {
              _showOnboarding = false;
              _onboardingChecked = true;
      ...
      // Nieuwe gebruiker of incompleet profiel: toon onboarding
      if (mounted) {
        setState(() {
      ...
      // Bij fout: skip onboarding, blokkeer app niet
      if (mounted) {
        setState(() {
          _showOnboarding = false;
          _onboardingChecked = true;
  ...
  Widget build(BuildContext context) {
    // Toon onboarding als die nog niet is afgerond
    if (_showOnboarding && _onboardingChecked) {
      return ClientOnboardingScreen(onComplete: _onOnboardingComplete);
    }
