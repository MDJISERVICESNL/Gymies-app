...
# Als script in project root staat: ROOT_DIR = SCRIPT_DIR; anders SCRIPT_DIR/..
ROOT_DIR="$SCRIPT_DIR"
[[ -f "$ROOT_DIR/pubspec.yaml" ]] || ROOT_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"
cd "$ROOT_DIR"
...
flutter build web --base-href "$BASE_HREF" --pwa-strategy=none -O4 --source-maps

echo "[1b/3] Force disable stale service worker clients"
python3 - <<'PY'
...
if marker not in content:
  ...
  <script>
    (function() {
      if (window.__gymies_sw_cleanup_done__) return;
      window.__gymies_sw_cleanup_done__ = true;
