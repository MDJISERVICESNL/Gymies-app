...
final class GymiesSupportController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        // N-020 FIXED: IDOR voorkomen — alleen eigen tickets
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        $allowedCategories = [
            'general', 'booking', 'payment', 'account', 'technical', 'other',
            'trainer', 'app', 'billing', 'refund', 'complaint', 'feedback',
            'schedule', 'profile', 'cancellation', 'review', 'group_session',
        ];
        $rawCategory = strtolower(trim((string) ($request->input('category') ?? 'general')));
        $category    = in_array($rawCategory, $allowedCategories, true) ? $rawCategory : 'general';
    ...
    // ──────────────────────────────────────────────────────────────────────────
    // PUBLIC (geen auth vereist) — contactformulier website
    // ──────────────────────────────────────────────────────────────────────────

    /**
     * Publiek contactformulier: maakt support ticket aan zonder ingelogd te zijn.
     * Rate-limited via middleware (max 5/min per IP).
     */
    public function publicContact(Request $request): JsonResponse
    {
        ...
        $category = $categoryMap[strtolower($subject)] ?? 'general';

        $insert = [
            'user_id'    => 0, // gast (niet ingelogd)
            'subject'    => '[Website] ' . ucfirst($subject),
            'category'   => $category,
            'priority'   => 'medium',
            'status'     => 'new',
            'created_at' => now(),
        ...
        if (Schema::hasColumn('gymies_support_tickets', 'submitter_name')) {
            $insert['submitter_name']  = $name;
            $insert['submitter_email'] = $email;
        }
