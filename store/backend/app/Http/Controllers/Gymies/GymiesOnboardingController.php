...
final class GymiesOnboardingController extends Controller
{
    ...
    public function uploadDocument(Request $request): JsonResponse
    {
        ...
        // S-001/S-002: Magic-byte validatie — MIME-type header alleen is niet genoeg.
        // PHP finfo leest de échte bestandsinhoud; voorkomt dat een .php-bestand als .jpg
        // wordt opgeslagen en via de public-URL als servercode wordt uitgevoerd.
        $allowedMagicMimes = ['application/pdf', 'image/jpeg', 'image/png'];
        if (function_exists('finfo_open')) {
            $finfo = finfo_open(FILEINFO_MIME_TYPE);
            $detectedMime = finfo_file($finfo, $file->getRealPath());
            finfo_close($finfo);
            if (!in_array($detectedMime, $allowedMagicMimes, true)) {
                return response()->json(['message' => 'Bestandstype niet toegestaan (magic-byte check mislukt).'], 422);
            }
        }

        // S-001: Sla op met een willekeurige, niet-raadbare bestandsnaam en een veilige extensie
        // zodat het bestand nooit als executable kan worden bereikt via de public URL.
        ...
        $path = $file->storeAs('gymies/documents/' . $user->id, $safeName, 'local');
    ...
    /**
     * S-055: CSRF OAuth State Reuse — controleer dat state aan sessie gekoppeld is bij aanvraag.
     * Deze helper valideert trainer_user_id binnenin callback; startMollieConnectBuildResponse slaat state op.
     */
    private function validateMollieStateForTrainer(string $state, int $trainerId): bool
    {
        if (!Schema::hasTable('gymies_mollie_oauth_states')) {
            return false;
        }
        $row = DB::table('gymies_mollie_oauth_states')->where('state', $state)->first();
        if (!$row) {
            return false;
        }
        // State moet aan dezelfde trainer hangen die nu ingelogd is
        return (int) $row->trainer_user_id === $trainerId;
    }
    ...
    public function mollieConnectCallback(Request $request): Response|JsonResponse
    {
        ...
        // S-054: Race Condition — controleer of state al gebruikt is (eenmalig gebruik)
        if (Schema::hasTable('gymies_mollie_oauth_states')) {
            $stateRecord = DB::table('gymies_mollie_oauth_states')->where('state', $state)->first();
            if ($stateRecord && !empty($stateRecord->used_at)) {
                logger()->warning('Gymies Mollie callback: state already used (replay attack?)', [
                    'state_prefix' => substr($state, 0, 8),
                    'trainer_user_id' => $trainerId,
                    'used_at' => $stateRecord->used_at,
                ]);
                return response()->json([
                    'message' => 'State is al gebruikt. Dit kan een replay-aanval zijn. Start opnieuw.',
                ], 422);
            }
        }
        ...
        // S-054: Race Condition — gebruik transactie + lockForUpdate om race conditions voorkomen
        $saved = false;
        if ($profileId && Schema::hasColumn('gymies_trainer_profiles', 'mollie_profile_id')) {
            DB::beginTransaction();
            try {
                // Vergrendel profiel-rij voor deze trainer
                $profile = DB::table('gymies_trainer_profiles')
                ...
                if ($profile) {
                    // Controleer of profiel_id al gezet is (voorkomen dubbele koppeling)
                    if (!empty($profile->mollie_profile_id)) {
                        logger()->warning('Gymies Mollie callback: profiel al gekoppeld', [
                            'trainer_user_id' => $trainerId,
                            'existing_profile_id' => $profile->mollie_profile_id,
                ...
                DB::rollBack();
                logger()->error('Gymies Mollie callback: transaction failed', [
                    'trainer_user_id' => $trainerId,
                    'error' => $e->getMessage(),
                ]);
                return response()->json(['message' => 'Fout bij opslaan profiel.'], 500);
            }
        }

        // Eenmalig consumeren zodra we token en profiel hebben verwerkt — voorkomt hergebruik state/code
        Cache::forget(self::MOLLIE_OAUTH_STATE_CACHE_PREFIX . $state);
        if (Schema::hasTable('gymies_mollie_oauth_states')) {
            // Mark state as used instead of deleting (audit trail + prevent reuse)
            DB::table('gymies_mollie_oauth_states')->where('state', $state)->update([
                'used_at' => now(),
            ]);
        }
        ...
        // Browser-request (Mollie redirect): HTML met redirect terug naar Flutter web app.
        // Bij popup: navigeer de opener (heeft sessie/token) en sluit popup — voorkomt uitloggen.
        $accept = $request->header('Accept', '');
        if (str_contains($accept, 'text/html')) {
            // Redirect naar Flutter web app (gymiesapp.nl), NIET naar API-domein (gymies.nl)
...
(function(){
  ...
  if (window.opener && !window.opener.closed) {
    ...
    public function selectPlan(Request $request): JsonResponse
    {
        ...
        $request->validate([
            'plan_slug'       => 'required|string|in:starter,pro,studio,pro_plus',
            'ambassador_code' => 'nullable|string|max:20|regex:/^[A-Z0-9]+$/i',
        ]);
        ...
        $ambassadorCode = strtoupper(trim((string) $request->input('ambassador_code', '')));
        try {
            $result = $subscriptionController->createSubscriptionForTrainer((int) $user->id, (int) $plan->id, $ambassadorCode);
        } catch (\Throwable $e) {
