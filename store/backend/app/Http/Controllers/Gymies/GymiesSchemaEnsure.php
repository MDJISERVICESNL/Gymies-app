  ...
  ADD COLUMN grace_period_ends_at TIMESTAMP NULL DEFAULT NULL AFTER resumed_at
    ...
    public static function reviewsTable(): bool
    {
        ...
        if (Schema::hasTable($table)) {
            // Zorg dat photo_url kolom bestaat (toegevoegd in v2)
            if (!Schema::hasColumn($table, 'photo_url')) {
                try {
                    DB::statement("ALTER TABLE {$table} ADD COLUMN photo_url VARCHAR(512) DEFAULT NULL AFTER is_anonymous");
                } catch (\Throwable) {}
            }
            return true;
        }
