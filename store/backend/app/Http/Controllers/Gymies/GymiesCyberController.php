...
class GymiesCyberController extends Controller
{
    ...
    public function webhooks(): JsonResponse
    {
        ...
        // Alert-drempels
        $failedRate = ($webhookRequests && $webhookRequests->total > 0)
            ? round(($webhookRequests->failed / $webhookRequests->total) * 100, 1)
            : 0;
        $avgMs = $webhookRequests->avg_ms ?? 0;

        $alerts = [];
        if ($failedRate > 10) {
            $alerts[] = "Hoog webhook failure rate: {$failedRate}% (>10%)";
        }
        if ($avgMs > 2000) {
            $alerts[] = "Trage webhook verwerking: {$avgMs}ms gemiddeld (>2000ms)";
        }
        if ($pendingTransactions > 5) {
            $alerts[] = "{$pendingTransactions} betalingen wachten >30min op webhook";
        }
        if ($webhookRequests && $webhookRequests->total === 0) {
            // Geen webhook calls in 24 uur kan normaal zijn bij lage traffic
            $alerts[] = 'Geen webhook calls in de afgelopen 24 uur';
        }

        // Reconciliatie: betalingen die betaald zijn bij Mollie maar niet in ons systeem
        $unreconciledCount = 0;
        if (Schema::hasTable('gymies_webhook_events')) {
            $unreconciledCount = DB::table('gymies_webhook_events')
                ->where('status', 'paid')
                ->whereNotExists(function ($q) {
                    $q->select(DB::raw(1))
                        ->from('gymies_payment_transactions')
                        ->whereColumn('gymies_payment_transactions.provider_transaction_id', 'gymies_webhook_events.provider_id')
                        ->where('gymies_payment_transactions.status', 'paid');
                })
                ->where('created_at', '>=', Carbon::now()->subHours(24))
                ->count();
        }
        if ($unreconciledCount > 0) {
            $alerts[] = "{$unreconciledCount} Mollie betalingen niet gereconcilieerd";
        }
        ...
        return response()->json([
            ...
            'unreconciled_payments'  => $unreconciledCount,
