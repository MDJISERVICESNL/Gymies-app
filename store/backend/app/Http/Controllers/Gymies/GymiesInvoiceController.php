...
final class GymiesInvoiceController extends Controller
{
    ...
    public function trainerInvoices(Request $request): JsonResponse
    {
        ...
        if (!Schema::hasTable('gymies_trainer_invoices')) {
            return response()->json(['invoices' => [], 'total_count' => 0]);
        }

        // S-098: Hard-coded Pagination Limit — Implementeer pagering in plaats van fixed limit(100)
        $page = max(1, (int) $request->query('page', 1));
        $perPage = 50;
        $offset = ($page - 1) * $perPage;

        // S-086: Intentional information disclosure — trainers legitimately need client emails for their invoices
    ...
    public function clientInvoiceDownload(Request $request, int $id): \Symfony\Component\HttpFoundation\Response|JsonResponse
    {
        ...
        if ($pdfUrl !== '' && $publicBase !== null && str_starts_with($pdfUrl, '/storage/')) {
            ...
            // S-009: Path traversal preventie – realpath() lost ../ en symlinks op.
            // Controleer dat het opgeloste pad écht binnen de storage-map valt.
            $realAbs  = realpath($abs);
            $realBase = realpath($publicBase);
            if (
                $realAbs !== false &&
                $realBase !== false &&
                str_starts_with($realAbs, $realBase . DIRECTORY_SEPARATOR) &&
                is_file($realAbs) &&
                is_readable($realAbs)
            ) {
                $ext  = strtolower(pathinfo($realAbs, PATHINFO_EXTENSION));
                $name = ($row->invoice_number ?? 'factuur') . '.' . ($ext === 'pdf' ? 'pdf' : 'html');
                return response()->download($realAbs, $name);
            }
        }
    ...
    public static function generateForBooking(object $booking): ?int
    {
        ...
        // S-084: Integer overflow check before VAT calculation
        if ($amountCents > PHP_INT_MAX / 100) {
            return null; // Amount too large to safely process
        }

        $vatPercent = 21;
        // S-084: Use integer arithmetic to avoid float rounding issues (21% VAT = 21/121)
        $vatCents = intdiv($amountCents * $vatPercent, 121);
    ...
    public function trainerInvoicesQuarterZip(Request $request): \Symfony\Component\HttpFoundation\Response|JsonResponse
    {
        ...
        // S-010: Begrens het aantal facturen per ZIP-verzoek om DoS/ZIP-bomb te voorkomen.
        $maxInvoicesPerZip = 200;
        ...
        $publicBase = function_exists('storage_path') ? storage_path('app/public') : null;
        $pdfAdded = [];
        // S-010: Houd totale ongecomprimeerde grootte bij (max 100 MB).
        ...
        foreach ($rows as $r) {
            ...
            if ($pdfUrl !== '' && $publicBase !== null && str_starts_with($pdfUrl, '/storage/')) {
                $rel    = substr($pdfUrl, strlen('/storage/'));
                $abs    = $publicBase . DIRECTORY_SEPARATOR . str_replace('/', DIRECTORY_SEPARATOR, $rel);
                // S-009: path-traversal check ook in ZIP-generator
                $realAbs  = realpath($abs);
                $realBase = realpath($publicBase);
                if (
                    $realAbs !== false &&
                    ...
                    $ext      = strtolower(pathinfo($realAbs, PATHINFO_EXTENSION));
                    $fileSize = filesize($realAbs);
                    // S-010: Sla over als het ZIP-totaal de limiet overschrijdt
                    if (($ext === 'pdf' || $ext === 'html') && ($totalUncompressedBytes + $fileSize) <= $maxZipBytes) {
                        $zip->addFile($realAbs, 'factuur_' . $index . '_' . $safeName . '.' . $ext);
