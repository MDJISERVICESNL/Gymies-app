...
final class GymiesBookingController extends Controller
{
    ...
    private function doIndex(Request $request): JsonResponse
    {
        ...
        if (Schema::hasTable('gymies_booking_reviews')) {
            ...
            if (!empty($completedIds)) {
                $reviewedBookingIds = DB::table('gymies_booking_reviews')->whereIn('booking_id', $completedIds)->pluck('booking_id')->map(fn ($id) => (int) $id)->all();
            }
        }
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        // Feature flag: buddy_bookings — klant kan een buddy uitnodigen
        $buddyEnabled = GymiesFeatureFlags::isEnabled('buddy_bookings', (int) $user->id, 'client');
        if ($buddyEnabled) {
            $validationRules['buddy_user_id'] = 'nullable|integer|exists:gymies_users,id';
        }

        $request->validate($validationRules);

        // Als buddy_bookings flag uit staat maar client stuurt buddy_user_id mee → negeer
        $buddyUserId = ($buddyEnabled && $request->filled('buddy_user_id'))
            ? (int) $request->input('buddy_user_id')
            : null;
        ...
        $availabilityError = $this->validateAgainstAvailabilitySlots($trainerId, $scheduledAt, $duration);
        if ($availabilityError instanceof JsonResponse) {
        ...
        try {
            ...
            // Feature flag: buddy_bookings — sla buddy op als kolom bestaat
            if ($buddyUserId !== null && Schema::hasColumn('gymies_bookings', 'buddy_user_id')) {
                $insertPayload['buddy_user_id'] = $buddyUserId;
            }

            // Beschikbaarheid metadata — bereken vanuit scheduled_at
            $scheduledCarbon = Carbon::parse($scheduledAt);
            if (Schema::hasColumn('gymies_bookings', 'slot_date')) {
                $insertPayload['slot_date'] = $scheduledCarbon->format('Y-m-d');
            }
            if (Schema::hasColumn('gymies_bookings', 'day_of_week')) {
                $insertPayload['day_of_week'] = (int) $scheduledCarbon->format('N');
            }
            if (Schema::hasColumn('gymies_bookings', 'week_number')) {
                $insertPayload['week_number'] = (int) $scheduledCarbon->format('W');
            }
        ...
        if (property_exists($row, 'buddy_user_id') && $row->buddy_user_id !== null) {
            $data['buddy_user_id'] = (string) $row->buddy_user_id;
        }
        if (property_exists($row, 'payment_method') && $row->payment_method !== null) {
            $data['payment_method'] = (string) $row->payment_method;
        }
        if (property_exists($row, 'slot_date') && $row->slot_date) {
            $data['slot_date'] = $row->slot_date;
            $data['day_of_week'] = property_exists($row, 'day_of_week') ? (int) $row->day_of_week : null;
            $data['week_number'] = property_exists($row, 'week_number') ? (int) $row->week_number : null;
        }
    ...
    private function doStoreDirectBook(GymiesDirectBookingRequest $request): JsonResponse
    {
        $user = $request->attributes->get('gymies_user');
        if (!$user) {
            return response()->json(['message' => 'Unauthorized'], 401);
        ...
        // If no service found or service_id not provided, use default from trainer profile
        if ($amountCents === 0 && Schema::hasTable('gymies_trainer_profiles')) {
            $profileCols = DB::getSchemaBuilder()->getColumnListing('gymies_trainer_profiles');
            $selectCols = array_intersect(['session_price_cents', 'hourly_rate_cents', 'trial_session_cents'], $profileCols);
            if (!empty($selectCols)) {
                $profile = DB::table('gymies_trainer_profiles')
                    ->where('user_id', $trainerId)
                    ->first($selectCols);
                if ($profile) {
                    // Prioriteit: session_price_cents → hourly_rate_cents → trial_session_cents
                    // Gebruik property_exists() i.p.v. ??: voorkomt Undefined property warning in PHP 8.4
                    foreach (['session_price_cents', 'hourly_rate_cents', 'trial_session_cents'] as $priceCol) {
                        if (property_exists($profile, $priceCol) && (int) $profile->$priceCol > 0) {
                            $amountCents = (int) $profile->$priceCol;
                            break;
                        }
                    }
                }
        ...
        // Trainer-notificatie: laat trainer weten dat er een nieuwe boeking is
        if (Schema::hasTable('gymies_notification_queue')) {
            ...
            try {
                DB::table('gymies_notification_queue')->insert([
                    'user_id' => $trainerId,
                    'channel' => 'in_app',
                    'event_type' => 'new_booking_for_trainer',
                    'payload_json' => json_encode([
                        ...
                        'message' => "{$clientName} heeft een sessie geboekt op " . Carbon::parse($scheduledAt)->format('d-m H:i'),
                    ], JSON_UNESCAPED_UNICODE),
                    'scheduled_for' => now(),
                    'created_at' => now(),
                ]);
    ...
    public function reject(Request $request, string $id): JsonResponse
    {
        ...
        if ($user->role !== 'trainer') { return response()->json(['message' => 'Alleen trainers kunnen boekingen afwijzen.'], 403); }
        $booking = DB::table('gymies_bookings')->where('id', $id)->first();
        if (!$booking) { return response()->json(['message' => 'Boeking niet gevonden.'], 404); }
        if ((int) $booking->trainer_user_id !== (int) $user->id) { return response()->json(['message' => 'Niet toegestaan'], 403); }
        if ($booking->status !== 'pending') { return response()->json(['message' => 'Alleen aanvragen in afwachting kunnen worden afgewezen.'], 422); }
        DB::table('gymies_bookings')->where('id', $id)->update(['status' => 'cancelled', 'cancelled_at' => now(), 'cancelled_by_user_id' => $user->id, 'updated_at' => now()]);
        $this->insertStatusNotifications((int) $id, (int) $booking->client_user_id, (int) $booking->trainer_user_id, 'booking_rejected');
        $this->logAudit((int) $user->id, 'booking_rejected', 'booking', (int) $id, ['status' => 'pending'], ['status' => 'cancelled']);
        return $this->respondBookingById((int) $id);
    }

    // T-020 FIXED: ownership check aanwezig
    ...
    public function rescheduleRequest(Request $request, string $id): JsonResponse
    {
        ...
        $availabilityError = $this->validateAgainstAvailabilitySlots($trainerId, $newScheduledAt, $newDuration);
        if ($availabilityError instanceof JsonResponse) {
            return $availabilityError;
        }
    ...
    public function cancel(Request $request, string $id): JsonResponse
    {
        ...
        // V2: Trigger waitlist — bied vrijgekomen slot aan eerste wachtende
        try {
            $scheduledAt = Carbon::parse($booking->scheduled_at);
            $slotDate = $scheduledAt->format('Y-m-d');
            $slotTime = $scheduledAt->format('H:i');
            (new WaitlistService())->onBookingCancelled(
                $trainerUserId,
                $slotDate,
                $slotTime,
                (int) $booking->id,
            ...
            // Waitlist trigger mag cancel niet laten falen
        }
    ...
    public function storeReview(Request $request, string $id): JsonResponse
    {
        ...
        $request->validate([
            ...
            'photo'        => 'nullable|file|image|max:5120', // max 5 MB
        ]);

        // S-030: Information Disclosure Review — Filter query by client_user_id
        $booking = DB::table('gymies_bookings')
            ->where('id', $id)
            ->where('client_user_id', $user->id)
            ->first();
        ...
        GymiesSchemaEnsure::reviewsTable();
        if (!Schema::hasTable($table)) {
            return response()->json(['message' => 'Beoordelingen zijn nog niet beschikbaar.'], 503);
        }
        if (DB::table($table)->where('booking_id', $id)->exists()) {
            return response()->json(['message' => 'Je hebt deze sessie al beoordeeld.'], 422);
        }

        // Foto opslaan indien aanwezig — met magic-byte validatie en veilige extensie
        $photoUrl = null;
        if ($request->hasFile('photo') && $request->file('photo')->isValid()) {
            $file = $request->file('photo');

            // Max 5MB, alleen afbeeldingen
            if ($file->getSize() > 5 * 1024 * 1024) {
                return response()->json(['message' => 'Foto mag maximaal 5MB zijn.'], 422);
            }

            // Magic-byte MIME check — voorkom executable uploads vermomd als afbeelding
            ...
            if (!in_array($detectedMime, $allowedMimes, true)) {
                return response()->json(['message' => 'Alleen JPEG, PNG of WebP afbeeldingen toegestaan.'], 422);
            }

            // Veilige extensie op basis van daadwerkelijk MIME — nooit client-extensie vertrouwen
            $mimeToExt = ['image/jpeg' => 'jpg', 'image/png' => 'png', 'image/webp' => 'webp'];
            $safeExt = $mimeToExt[$detectedMime] ?? 'bin';
            $dir  = 'reviews/' . date('Y/m');
            $name = 'review_' . $id . '_' . time() . '.' . $safeExt;
            $path = $file->storeAs($dir, $name, 'public');
            $photoUrl = $path ? '/storage/' . $path : null;
        }

        $rating      = (int) $request->input('rating');
        $message     = $request->input('message') ? trim((string) $request->input('message')) : null;
    ...
    public function getReviewStatus(Request $request, string $id): JsonResponse
    {
        ...
        $hasReview = false;
        $rating = null;
        $reviewTable = 'gymies_booking_reviews';
        if (Schema::hasTable($reviewTable)) {
            $review = DB::table($reviewTable)->where('booking_id', $id)->first(['rating']);
    ...
    public function getBookingReview(Request $request, string $id): JsonResponse
    {
        ...
        $review = DB::table('gymies_booking_reviews')->where('booking_id', (int) $id)->first(['id', 'rating', 'message', 'photo_url', 'created_at']);
        if (!$review) {
            return response()->json(['data' => ['has_review' => false]]);
        }
        $responseText = null;
        ...
        return response()->json([
            'data' => [
                ...
                'message' => isset($review->message) ? (string) $review->message : null,
                'photo_url' => isset($review->photo_url) ? (string) $review->photo_url : null,
    ...
    public function storeReviewResponse(Request $request, string $id): JsonResponse
    {
        ...
        $review = DB::table('gymies_booking_reviews')->where('booking_id', (int) $id)->first(['id']);
        if (!$review) {
    ...
    /**
     * Valideer dat het gevraagde tijdslot binnen de beschikbaarheid van de trainer valt.
     */
    private function validateAgainstAvailabilitySlots(int $trainerId, string $scheduledAt, int $duration): ?JsonResponse
    {
        if (!Schema::hasTable('gymies_availability_slots')) {
            return null; // Geen beschikbaarheidstabel — overslaan
        }
        ...
        $hasIsActive = Schema::hasColumn('gymies_availability_slots', 'is_active');

        // Check of de trainer beschikbaarheid heeft ingesteld
        $countQuery = DB::table('gymies_availability_slots')
            ->where('trainer_user_id', $trainerId);
        if ($hasIsActive) {
            $countQuery->where('is_active', true);
        }
        $totalSlots = $countQuery->count();

        if ($totalSlots === 0) {
            return null; // Trainer heeft (nog) geen beschikbaarheid ingesteld — laat door
        }

        // Check eerst op exacte datum (slot_date), fallback naar weekdag
        $hasDateSlots = Schema::hasColumn('gymies_availability_slots', 'slot_date');
        $matchingSlot = null;

        if ($hasDateSlots) {
            $dateQuery = DB::table('gymies_availability_slots')
                ->where('trainer_user_id', $trainerId)
                ->where('slot_date', $slotDate)
                ->where('start_time', '<=', $startTime)
                ->where('end_time', '>=', $endTime);
            if ($hasIsActive) {
                $dateQuery->where('is_active', true);
            }
            $matchingSlot = $dateQuery->first();
        }

        if (!$matchingSlot) {
            $weekdayQuery = DB::table('gymies_availability_slots')
                ->where('trainer_user_id', $trainerId)
                ->where('day_of_week', $dayOfWeek)
                ->where('start_time', '<=', $startTime)
                ->where('end_time', '>=', $endTime);
            if ($hasIsActive) {
                $weekdayQuery->where('is_active', true);
            }
            $matchingSlot = $weekdayQuery->first();
        }

        if (!$matchingSlot) {
            return response()->json([
                'message' => 'Dit tijdslot valt buiten de beschikbaarheid van de trainer. Kies een ander moment.',
            ], 422);
        }

        // Check uitzonderingen (geblokkeerde datums)
        if (Schema::hasTable('gymies_availability_exceptions')) {
            $exCols = DB::getSchemaBuilder()->getColumnListing('gymies_availability_exceptions');
            // Zoek de juiste kolomnaam voor de datum (blocked_date, exception_date, date, ...)
            $dateCol = null;
            foreach (['blocked_date', 'exception_date', 'date', 'slot_date'] as $candidate) {
                if (in_array($candidate, $exCols)) {
                    $dateCol = $candidate;
                    break;
                }
            }
            if ($dateCol) {
                $exQuery = DB::table('gymies_availability_exceptions')
                    ->where('trainer_user_id', $trainerId)
                    ->where($dateCol, $slotDate);
                if (in_array('is_available', $exCols)) {
                    $exQuery->where('is_available', false);
                }
                $exception = $exQuery->first();
                if ($exception) {
                    $reason = property_exists($exception, 'reason') ? ($exception->reason ?? '') : '';
                    return response()->json([
                        'message' => 'Deze trainer is niet beschikbaar op ' . Carbon::parse($slotDate)->format('d-m-Y') . '.' . ($reason ? " Reden: {$reason}" : ''),
                    ], 422);
                }
            }
        }
    ...
    /** Hold duration in minutes for pre-booking slot reservation. */
    private const SLOT_HOLD_MINUTES = 5;

    /**
     * POST bookings/hold-slot
     * Reserveer een slot tijdelijk zodat niemand anders het kan boeken
     * terwijl de klant het betalingsproces doorloopt.
     */
    public function holdSlot(Request $request): JsonResponse
    {
        ...
        // Check of er al een actieve hold is voor dit slot
        $hasCol = Schema::hasColumn('gymies_bookings', 'reserved_until');
        if ($hasCol) {
            ...
            if ($existing) {
                // Als het dezelfde klant is, geef het bestaande hold terug
                if ((int) $existing->client_user_id === $user->id) {
                    $expiresIn = max(0, (int) now()->diffInSeconds($existing->reserved_until, false));
                    return response()->json([
                        'success' => true,
                        'data' => [
                            'hold_id'    => $existing->id,
                ...
                return response()->json([
                    'success' => false,
                    'message' => 'Dit tijdslot is al gereserveerd door een andere klant.',
                ], 409);
            }
        }

        // Check overlap met bestaande confirmed/pending bookings
        if ($this->hasConfirmedOverlap($trainerId, $scheduledAt, $duration)) {
            return response()->json([
                'success' => false,
        ...
        $expiresAt = now()->addMinutes(self::SLOT_HOLD_MINUTES);
        ...
        if ($hasCol) {
            $payload['reserved_until'] = $expiresAt;
        }

        $holdId = DB::table('gymies_bookings')->insertGetId($payload);

        return response()->json([
            'success' => true,
            'data' => [
                'hold_id'    => $holdId,
                'expires_in' => self::SLOT_HOLD_MINUTES * 60,
            ],
        ], 201);
    }
     ...
     * Laat een slot-hold los (klant navigeert weg zonder te boeken).
     */
    public function releaseSlot(Request $request, string $holdId): JsonResponse
    {
        $user = $request->user();

        $deleted = DB::table('gymies_bookings')
            ->where('id', (int) $holdId)
            ->where('client_user_id', $user->id)
            ->where('status', 'reserved')
            ->delete();
        ...
        return response()->json([
            'success' => $deleted > 0,
            'message' => $deleted > 0 ? 'Slot hold vrijgegeven.' : 'Hold niet gevonden of al verlopen.',
        ]);
    }

    /**
     * POST bookings/hold-slot/{holdId}/extend
     * Verleng een bestaande slot-hold met nog eens SLOT_HOLD_MINUTES.
     * Max 2 keer verlengen (afgedwongen door Flutter, maar ook server-side).
     */
    public function extendSlotHold(Request $request, string $holdId): JsonResponse
    {
        $user = $request->user();

        $hold = DB::table('gymies_bookings')
            ->where('id', (int) $holdId)
            ->where('client_user_id', $user->id)
            ->where('status', 'reserved')
            ->first();
        ...
        if (!$hold) {
            return response()->json([
                'success' => false,
                'message' => 'Hold niet gevonden.',
            ], 404);
        }

        // Check of reserved_until kolom bestaat
        if (!Schema::hasColumn('gymies_bookings', 'reserved_until')) {
            return response()->json([
                'success' => false,
                'message' => 'Extend niet ondersteund.',
            ], 400);
        }

        // Check of de hold al verlopen is
        if (isset($hold->reserved_until) && now()->gt($hold->reserved_until)) {
            // Verwijder verlopen hold
            DB::table('gymies_bookings')->where('id', (int) $holdId)->delete();
            return response()->json([
                'success' => false,
                'message' => 'Hold is al verlopen.',
            ], 410);
        }

        // Check of het slot nog vrij is (geen andere booking in de tussentijd)
        $trainerId   = (int) $hold->trainer_user_id;
        $scheduledAt = $hold->scheduled_at;
        $duration    = (int) ($hold->duration_minutes ?? 60);

        $hasOverlap = DB::table('gymies_bookings')
            ->where('trainer_user_id', $trainerId)
            ->where('id', '!=', (int) $holdId)
            ->whereIn('status', ['confirmed', 'pending'])
            ->where(function ($q) use ($scheduledAt, $duration) {
                $q->whereBetween('scheduled_at', [
                    $scheduledAt,
                    date('Y-m-d H:i:s', strtotime($scheduledAt) + ($duration * 60) - 1),
                ]);
            })
            ->exists();
        ...
        if ($hasOverlap) {
            ...
            return response()->json([
                'success' => false,
                'message' => 'Dit tijdslot is inmiddels geboekt.',
            ], 409);
        }

        // Verleng de hold
        $newExpiresAt = now()->addMinutes(self::SLOT_HOLD_MINUTES);
        DB::table('gymies_bookings')
            ->where('id', (int) $holdId)
            ->update([
        ...
        $expiresIn = self::SLOT_HOLD_MINUTES * 60;
