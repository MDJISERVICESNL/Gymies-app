...
final class GymiesTrainerOpsController extends Controller
{
    ...
    public function storefrontCmsGet(Request $request): JsonResponse
    {
        ...
        try {
            ...
            if (!$planCheck['allowed']) {
                return response()->json([
                    ...
                    'code'         => 'plan_storefront_locked',
                ], 403);
            }
        } catch (\Throwable $e) {
            \Log::warning('[storefrontCmsGet] PlanManager error, allowing access as fallback', [
                'user_id' => $user->id,
                'error'   => $e->getMessage(),
            ]);
            // Bij fout in plancheck: laat door (graceful degradation)
        }

        // ── Slug ophalen (voor embed code + QR) ─────────────────
        $slug = '';
        try {
            if (Schema::hasTable('gymies_trainer_profiles')) {
                if (Schema::hasColumn('gymies_trainer_profiles', 'custom_slug')) {
                    $slug = (string) DB::table('gymies_trainer_profiles')
                        ->where('user_id', (int) $user->id)
                        ->value('custom_slug');
                }
                if (empty($slug) && Schema::hasColumn('gymies_trainer_profiles', 'slug')) {
                    $slug = (string) DB::table('gymies_trainer_profiles')
                        ->where('user_id', (int) $user->id)
                        ->value('slug');
                }
            }
            // Fallback: genereer slug uit user_id
            if (empty($slug)) {
                $slug = 'trainer-' . (int) $user->id;
            }
        } catch (\Throwable $e) {
            $slug = 'trainer-' . (int) $user->id;
        }
        ...
        try {
            ...
            // Stories JSON decoderen
            $stories = [];
            if (Schema::hasColumn('gymies_trainer_storefront', 'success_stories_json')
                && !empty($row->success_stories_json)) {
            ...
            // Layout JSON decoderen
            $layout = null;
            if (Schema::hasColumn('gymies_trainer_storefront', 'layout_json')
                && !empty($row->layout_json)) {
            ...
            // Veilig kolommen uitlezen
            $safeCol = function (string $col) use ($row): ?string {
                return Schema::hasColumn('gymies_trainer_storefront', $col)
                    ? ($row->$col ?? null)
                    : null;
            };
            ...
            return response()->json([
                'data' => [
                    ...
                    'video_pitch_url'         => $safeCol('video_pitch_url'),
                    'instagram_handle'        => $safeCol('instagram_handle'),
                    'specializations_display' => $safeCol('specializations_display'),
                    'seo_title'               => $safeCol('seo_title'),
                    'seo_description'         => $safeCol('seo_description'),
                    'seo_keywords'            => $safeCol('seo_keywords'),
                    'layout'                  => $layout,
                ],
            ]);
        } catch (\Throwable $e) {
            \Log::error('[storefrontCmsGet] Fout bij ophalen storefront', [
                'user_id' => $user->id,
                'error'   => $e->getMessage(),
            ]);
            return response()->json(['data' => $emptyData]);
    ...
    public function storefrontCmsPut(Request $request): JsonResponse
    {
        ...
        try {
            ...
            \Log::warning('[storefrontCmsPut] PlanManager error, allowing access as fallback', ['error' => $e->getMessage()]);
        }
        if (!Schema::hasTable('gymies_trainer_storefront')) {
