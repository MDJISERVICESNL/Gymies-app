...
final class GymiesCheckinController extends Controller
{
    ...
    public function getCheckinQr(Request $request, string $bookingId): JsonResponse
    {
        ...
        $backupCode = $booking->checkin_backup_code ?? str_pad((string) random_int(0, 999999), 6, '0', STR_PAD_LEFT);
    ...
    public function manualCheckin(Request $request): JsonResponse
    {
        ...
        if ($booking->checkin_backup_code === null || $booking->checkin_backup_code !== $code) {
        ...
        try {
            ...
            if (Schema::hasColumn('gymies_users', 'is_suspended')) {
                DB::table('gymies_users')->where('id', $clientUserId)->update([
                    'is_suspended' => 1,
                    'suspended_reason' => 'Identiteitsfraude gemeld door trainer. Account geblokkeerd voor onderzoek.',
                    'suspended_at' => now(),
                ]);
            }
