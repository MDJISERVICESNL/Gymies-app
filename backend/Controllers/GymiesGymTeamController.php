...
final class GymiesGymTeamController extends GymiesGymController
{
    public function index(Request $request): JsonResponse
    {
        ...
        if (!Schema::hasTable('gym_teams')) {
            return response()->json(['data' => []]);
        }

        $orgId = (int) $ctx['organisation_id'];
        $rows = DB::table('gym_teams')
        ...
        $membersCount = [];
        if (Schema::hasTable('gym_team_members')) {
            $counts = DB::table('gym_team_members')
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        if ($parentTeamId !== null) {
            $parent = DB::table('gym_teams')
                ->where('id', $parentTeamId)
                ->where('organisation_id', $orgId)
                ->first();
            if (!$parent) {
                return response()->json(['message' => 'Parent team niet gevonden.'], 422);
            }
        }

        $sortOrder = (int) ($request->input('sort_order') ?? 0);

        $id = DB::table('gym_teams')->insertGetId([
        ...
        $row = DB::table('gym_teams')->where('id', $id)->first();
    ...
    public function update(Request $request, string $id): JsonResponse
    {
        ...
        if ($request->has('parent_team_id')) {
            ...
            if ($parentId === $teamId) {
                return response()->json(['message' => 'Team kan niet zijn eigen parent zijn.'], 422);
            }
        ...
        DB::table('gym_teams')->where('id', $teamId)->update($payload);
        $row = DB::table('gym_teams')->where('id', $teamId)->first();
    ...
    public function addMember(Request $request, string $teamId): JsonResponse
    {
        ...
        if (!Schema::hasTable('gym_teams') || !Schema::hasTable('gym_team_members')) {
            return response()->json(['message' => 'Teams niet beschikbaar.'], 422);
        }
        ...
        $exists = DB::table('gym_team_members')
            ->where('team_id', $tid)
            ->where('trainer_user_id', $trainerUserId)
            ->exists();
        if ($exists) {
        ...
        DB::table('gym_team_members')->insert([
            'team_id' => $tid,
    ...
    public function removeMember(Request $request, string $teamId, string $trainerUserId): JsonResponse
    {
        ...
        $deleted = DB::table('gym_team_members')
            ->where('team_id', $tid)
