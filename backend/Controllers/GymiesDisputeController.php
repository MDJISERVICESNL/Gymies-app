 ...
 *
 * Routes (all inside auth middleware group):
 *   GET    my-disputes              → index
 *   GET    my-disputes/{id}         → show
 *   POST   my-disputes/{id}/message → addMessage
 *   POST   bookings/{id}/dispute    → raise
 */
class GymiesDisputeController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        // Sorteer alles op datum (nieuwste eerst)
    ...
    public function show(Request $request, string $disputeId): JsonResponse
    {
        ...
        // Check of het een support ticket dispute is (id begint met "ticket_")
    ...
    private function showTicketDispute(Request $request, string $disputeId, int $userId): JsonResponse
    {
        ...
        // Map ticket velden naar dispute formaat
        $status = match (true) {
    ...
    public function addMessage(Request $request, string $disputeId): JsonResponse
    {
        ...
        // Controleer dat de user bij dit dispute hoort
        $d = DB::table('gymies_disputes as d')
            ->leftJoin('gymies_bookings as b', 'b.id', '=', 'd.booking_id')
            ->where('d.id', $id)
            ->where(function ($q) use ($userId) {
    ...
    public function raise(Request $request, string $bookingId): JsonResponse
    {
        ...
        // Controleer dat de boeking bij de user hoort
        ...
        // Check of er al een open dispute is voor deze boeking
