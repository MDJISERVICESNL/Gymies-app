...
final class GymiesSpoedInvalController extends Controller
{
    ...
    public function respondToOffer(Request $request, string $offerId): JsonResponse
    {
        ...
        if ($accept) {
            ...
            DB::beginTransaction();
            try {
                // Race condition: conditional update – als iemand anders net accepteerde, 0 rows
                $rowsUpdated = DB::table('gymies_spoed_inval_requests')
                ...
                if ($rowsUpdated === 0) {
                    DB::rollBack();
                    return response()->json([
                        'message' => 'Helaas, een andere trainer was je net voor.',
                    ], 409);
                }
    ...
    private function findAvailableCandidatesWithBatch(int $excludeTrainerId, $bookings, string $region, string $specialty): array
    {
        ...
        if ($region !== '') {
            $trainersQuery->where(function ($q) use ($region) {
                $q->where('p.region', 'like', '%' . $region . '%')
                    ->orWhere('p.region', 'like', $region . '%');
            });
        }
        if ($specialty !== '') {
            $trainersQuery->where(function ($q) use ($specialty) {
                $q->where('p.specialty', 'like', '%' . $specialty . '%')
                    ->orWhere('p.specialty', 'like', $specialty . '%');
            });
        }
