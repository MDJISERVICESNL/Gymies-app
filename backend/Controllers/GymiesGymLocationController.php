...
final class GymiesGymLocationController extends GymiesGymController
{
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        $id = DB::table('gym_locations')->insertGetId([
        ...
        $row = DB::table('gym_locations')->where('id', $id)->first();
    ...
    public function update(Request $request, string $id): JsonResponse
    {
        ...
        DB::table('gym_locations')->where('id', $id)->update($payload);
        $row = DB::table('gym_locations')->where('id', $id)->first();
    ...
    public function destroy(Request $request, string $id): JsonResponse
    {
        ...
        $orgId = (int) $ctx['organisation_id'];
        $deleted = DB::table('gym_locations')
            ->where('id', $id)
            ->where('organisation_id', $orgId)
            ->delete();
    ...
    public function blocks(Request $request, string $id): JsonResponse
    {
        ...
        if (!Schema::hasTable('gym_locations') || !Schema::hasTable('gym_location_blocks')) {
            return response()->json(['data' => []]);
        }

        $orgId = (int) $ctx['organisation_id'];
        $location = DB::table('gym_locations')
            ->where('id', $id)
            ->where('organisation_id', $orgId)
            ->first();
        if (!$location) {
        ...
        $to = $request->query('to');
        $query = DB::table('gym_location_blocks')->where('location_id', $id);
        if ($from && $to) {
    ...
    public function storeBlock(Request $request, string $id): JsonResponse
    {
        ...
        $blockId = DB::table('gym_location_blocks')->insertGetId([
            'location_id' => (int) $id,
        ...
        $row = DB::table('gym_location_blocks')->where('id', $blockId)->first();
        return response()->json([
            'data' => [
                'id' => (string) $row->id,
    ...
    public function destroyBlock(Request $request, string $id, string $blockId): JsonResponse
    {
        ...
        if (!Schema::hasTable('gym_location_blocks')) {
            return response()->json(['message' => 'Blokkades niet beschikbaar.'], 422);
        }

        $orgId = (int) $ctx['organisation_id'];
        $block = DB::table('gym_location_blocks as b')
            ->join('gym_locations as l', 'l.id', '=', 'b.location_id')
            ->where('b.id', $blockId)
        ...
        DB::table('gym_location_blocks')->where('id', $blockId)->delete();
        return response()->json(['ok' => true], 200);
    }
    ...
    public function conflicts(Request $request, string $id): JsonResponse
    {
        ...
        // Blokkades
        if (Schema::hasTable('gym_location_blocks')) {
            $blocks = DB::table('gym_location_blocks')
