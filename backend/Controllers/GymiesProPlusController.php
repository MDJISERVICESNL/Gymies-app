...
final class GymiesProPlusController extends Controller
{
    ...
    public function sendNewsletter(Request $request): JsonResponse
    {
        ...
        // Sanitize: subject altijd plain text, body mag veilige HTML bevatten
...
HTML;
    ...
    {
        // Client naam altijd escapen; body is al gesanitized via strip_tags met allowlist
...
<head>
    ...
    <style>
        ...
        .content a { color: #FEBE23; }
        .content blockquote { border-left: 3px solid #FEBE23; padding-left: 12px; margin-left: 0; color: #555; }
...
HTML;
    ...
    /**
     * Get widget stats (sessions booked, active clients, revenue)
     */
    public function widgetStats(Request $request): JsonResponse
    {
        $user = $request->attributes->get('gymies_user');
        if (!$user || $user->role !== 'trainer') {
            return response()->json(['message' => 'Unauthorized'], 403);
        ...
        if (!Schema::hasTable('gymies_bookings')) {
            return response()->json(['message' => 'Bookings table not found'], 500);
        }

        // Total sessions booked via widget
        $totalSessions = DB::table('gymies_bookings')
            ->where('trainer_user_id', (int) $user->id)
            ->count();

        // Active clients (booked in last 30 days)
        $activeClients = DB::table('gymies_bookings')
            ->where('trainer_user_id', (int) $user->id)
        ...
        $totalRevenue = DB::table('gymies_bookings')
            ->where('trainer_user_id', (int) $user->id)
            ->sum('amount_cents') ?? 0;
    ...
    /**
     * Schedule newsletter to send at specific time
     */
    public function scheduleNewsletter(Request $request): JsonResponse
    {
        $user = $request->attributes->get('gymies_user');
        if (!$user || $user->role !== 'trainer') {
            return response()->json(['message' => 'Unauthorized'], 403);
        ...
        $validated = $request->validate([
            'subject' => ['required', 'string', 'max:200', 'regex:/^[^<>{}]*$/'],
            'body'    => ['required', 'string', 'min:10', 'max:50000'],
            'send_at' => ['required', 'date_format:Y-m-d H:i:s', 'after:now'],
        ]);
        ...
        // Create newsletter record with scheduled_at timestamp
        $newsletter = TrainerNewsletter::create([
            'trainer_user_id' => (int) $user->id,
            'subject' => $validated['subject'],
            'body' => $validated['body'],
            'scheduled_at' => $validated['send_at'],
        ]);
