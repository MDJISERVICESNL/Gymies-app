...
final class GymiesAuthController extends Controller
{
    ...
    {
        $n = trim((string) config('gymies.mail_brand', ''));
        if ($n !== '') {
            return $n;
        }
    ...
    {
        $forced = rtrim(trim((string) config('gymies.public_url', '')), '/');
        if ($forced !== '') {
            return $forced;
        }
    ...
    public function register(GymiesRegisterRequest $request): JsonResponse
    {
        ...
        // Gym invite: alleen voor trainers; valideer token vooraf (buiten transaction — read-only)
        $gymInvite = null;
        if ($gymInviteToken !== '' && Schema::hasTable('gym_invites') && Schema::hasTable('gymies_organisations')) {
            $inviteRow = DB::table('gym_invites as i')
        ...
        // ──────────────────────────────────────────────────────────────
        //  TRANSACTION: user insert + referral + gym invite koppeling
        ...
        try {
            ...
            // Referral: code koppelt uitnodiger (referrer) aan nieuwe gebruiker. Eén code = één registratie.
            ...
            if ($referralCode !== '' && Schema::hasTable('gymies_referrals')) {
                ...
                if ($referral) {
                    if ((int) $referral->referrer_user_id === (int) $id) {
                        // Zelf-uitnodiging voorkomen — registratie gaat gewoon door
                    } elseif ($referral->referred_user_id !== null && $referral->referred_user_id !== '') {
                        if (function_exists('logger')) {
                            logger()->info('Gymies register: referral code already used', [
                                'referral_id' => $referral->id,
                                'code' => $referralCodeNorm,
                            ]);
                        }
                    } elseif (($referral->status ?? 'pending') !== 'pending') {
                        // Verlopen of afgerond — niet opnieuw koppelen
                    } else {
            ...
            // Ambassador: als referral_code een ambassador discount_code is, registreer conversie
            if ($referralCode !== '') {
                $referralCodeNorm = strtoupper($referralCode);
                // Check of dit een ambassador-code is (los van het referral-systeem)
                if (Schema::hasTable('gymies_ambassadors')) {
                    $ambMatch = DB::table('gymies_ambassadors')
                        ->where('discount_code', $referralCodeNorm)
                        ->where('is_active', 1)
                        ->first(['id', 'user_id']);
                    if ($ambMatch) {
                        // Link user to ambassador als e-mail overeenkomt
                        GymiesAmbassadorController::linkUserToAmbassador($id, $email);

                        // Registreer trainer-conversie als de nieuwe user een trainer is
                        $newRole = $request->input('role');
                        if ($newRole === 'trainer' && ((int) ($ambMatch->user_id ?? 0)) !== (int) $id) {
                            GymiesAmbassadorController::recordTrainerConversion($referralCodeNorm, $id);
                        }
                    }
                }
            }

            // Gym invite: koppel nieuwe trainer aan organisatie en verhoog used_count
            if ($gymInvite !== null && Schema::hasTable('gymies_organisation_trainers')) {
                $orgId = (int) $gymInvite->organisation_id;

                // Lock invite row om race condition te voorkomen bij gelijktijdige registraties
                $freshInvite = DB::table('gym_invites')->where('id', $gymInvite->id)->lockForUpdate()->first();
                if ($freshInvite && ($freshInvite->max_uses === null || (int) $freshInvite->used_count < (int) $freshInvite->max_uses)) {
                    ...
                    DB::table('gym_invites')->where('id', $gymInvite->id)->increment('used_count');
                } else {
                    DB::rollBack();
                    throw ValidationException::withMessages(['gym_invite_token' => ['Deze uitnodiging is inmiddels volledig gebruikt.']]);
                }
            }
            ...
            DB::rollBack();
            if (function_exists('logger')) {
                logger()->error('Gymies register: transaction failed', ['email' => $email, 'error' => $e->getMessage()]);
            }
            return response()->json(['message' => 'Registratie is mislukt. Probeer het opnieuw.'], 500);
        }

        // Verificatie-e-mail (buiten transaction — mail mag niet de registratie blokkeren)
        try {
            ...
            if (Schema::hasColumn('gymies_email_verification_codes', 'link_token')) {
                $verificationInsert['link_token'] = $linkToken;
            }
            DB::table('gymies_email_verification_codes')->insert($verificationInsert);
    ...
    public function resendVerificationCode(Request $request): JsonResponse
    {
        ...
        return response()->json(['message' => 'We hebben een nieuwe code gestuurd naar je e-mailadres. De code is 15 minuten geldig.']);
    }
    ...
    {
        $key = trim((string) (config('services.brevo.api_key') ?? config('gymies.brevo_api_key', '')));
        return $key !== '' ? $key : null;
    }
    ...
    private function sendTransactionalMail(string $toEmail, string $subject, string $textBody, string $htmlBody): bool
    {
        ...
        try {
            ...
            if (function_exists('logger')) {
                logger()->warning('Gymies transactional mail send failed', ['email' => $toEmail, 'error' => $e->getMessage()]);
            }
        }
        return false;
    }
    ...
    private function sendPasswordResetEmail(string $email, string $token, \DateTimeInterface $expiresAt): void
    {
        ...
        $resetUrl = $baseUrl . '/wachtwoord-reset?token=' . urlencode($token);
        ...
        $inner = '<p style="margin:0 0 16px;">Je hebt een wachtwoordreset aangevraagd. Klik op de knop om een nieuw wachtwoord in te stellen.</p>'
            . '<p style="margin:0 0 20px;"><a href="' . $resetUrlEsc . '" style="display:inline-block;background:#FF8A00;color:#ffffff;text-decoration:none;padding:14px 28px;border-radius:12px;font-weight:600;">Nieuw wachtwoord instellen</a></p>'
            . '<p style="margin:0 0 8px;color:#5C6773;font-size:13px;">Werkt de knop niet? Kopieer deze link in je browser:</p>'
            . '<p style="margin:0;word-break:break-all;font-size:13px;"><a href="' . $resetUrlEsc . '" style="color:#0B1F3A;">' . $resetUrlEsc . '</a></p>'
            . '<p style="margin:20px 0 0;color:#5C6773;font-size:13px;">Deze link is <strong>1 uur</strong> geldig.</p>';
        $htmlBody = $this->mailHtmlWrapper($subject, $inner, $appName);
        ...
        try {
            if (config('mail.default') && class_exists(\Illuminate\Mail\Mailer::class)) {
                Mail::raw($textBody, function ($message) use ($email, $subject): void {
    ...
    private function sendVerificationCodeEmail(string $email, string $code, \DateTimeInterface $expiresAt, ?string $linkToken = null): bool
    {
        ...
        $baseUrl = $this->mailPublicBaseUrl();
        $verifyUrl = $linkToken
            ? $baseUrl . '/verifieer-email?token=' . rawurlencode($linkToken)
            : $baseUrl . '/verifieer-email?email=' . rawurlencode($email);
        ...
        // Plain text: leesbaar zonder HTML (fallback)
        $textBody = "Hoi,\n\n"
            . "Welkom bij {$appName}! Hier is je code om je e-mail te bevestigen:\n\n"
            . "  {$code}\n\n"
            . "De code is 15 minuten geldig.\n\n"
            . ($linkToken ? "Klik op deze link om direct in te loggen (geen code nodig):\n{$verifyUrl}\n\n" : '')
        ...
        $buttonText = $linkToken ? 'Direct inloggen (1 klik)' : 'Naar verificatie';
        // Altijd code tonen + knop. Knop = magic link (direct inloggen) als linkToken, anders naar verificatiepagina.
        $codeBlock = '<div style="background:linear-gradient(180deg,#F8FAFC 0%,#F2F5F9 100%);border-radius:14px;padding:24px 20px;text-align:center;margin:0 0 22px;border:1px solid #E2E8F0;">'
            . '<div style="color:#5C6773;font-size:11px;text-transform:uppercase;letter-spacing:0.12em;margin-bottom:10px;">Jouw code · 15 minuten geldig</div>'
        ...
        $introText = $linkToken
            ? 'Welkom bij <strong style="color:#0B1F3A;">' . $appNameEsc . '</strong>. Klik op de knop om direct in te loggen, of vul de code hieronder in als je dat liever doet.'
            : 'Welkom bij <strong style="color:#0B1F3A;">' . $appNameEsc . '</strong>. Vul de code hieronder in de app in om je account te activeren.';
        $inner = '<p style="margin:0 0 12px;font-size:16px;">Hoi,</p>'
            . '<p style="margin:0 0 18px;">' . $introText . '</p>'
            . $codeBlock
            . '<p style="margin:0 0 18px;text-align:center;"><a href="' . $verifyUrlEsc . '" style="display:inline-block;background:#FF8A00;color:#ffffff;text-decoration:none;padding:16px 32px;border-radius:14px;font-weight:700;box-shadow:0 4px 14px rgba(255,138,0,0.35);">' . htmlspecialchars($buttonText, ENT_QUOTES, 'UTF-8') . '</a></p>'
            ...
            . '<p style="margin:0;color:#5C6773;font-size:13px;">Nieuwe code nodig? In de app: <strong>Code opnieuw sturen</strong>.</p>';
        $htmlBody = $this->mailHtmlWrapper('Bevestig je e-mail', $inner, $appName);
        ...
        try {
            if (config('mail.default') && class_exists(\Illuminate\Mail\Mailer::class)) {
                // Geen Mail::raw — anders verlies je HTML in de inbox
    ...
    private function userToArray(object $user): array
    {
        ...
        return [
            ...
            ...$this->ambassadorFields((int) $user->id),
        ];
    }

    /**
     * Ambassador-data voor auth/me response.
     * Retourneert is_ambassador, tier, code, share_url, verdiensten, etc.
     */
    private function ambassadorFields(int $userId): array
    {
        $ambData = GymiesAmbassadorController::getProfileData($userId);
        if ($ambData === null) {
            return [
                'is_ambassador' => false,
                'ambassador' => null,
    ...
    public function claimCorporateCode(Request $request): JsonResponse
    {
        ...
        $alreadyLinked = DB::table('gymies_users')->where('id', (int) $user->id)->value('corporate_organisation_id');
        if ($alreadyLinked) {
            return response()->json(['message' => 'Je bent al gekoppeld aan een bedrijf.'], 422);
        }
    ...
    /**
     * POST ws-ticket
     * Genereert een kortstondig WebSocket-ticket (60 seconden geldig).
     * De Flutter app gebruikt dit ticket i.p.v. het lange-termijn access_token
     * in de WebSocket query string, zodat het access_token niet wordt blootgesteld.
     */
    public function wsTicket(Request $request): JsonResponse
    {
        ...
        // Sla ticket op in gymies_sessions als ws_ticket (hergebruik bestaande tabel)
        // of in cache als dat beschikbaar is
        if (Schema::hasTable('gymies_ws_tickets')) {
            DB::table('gymies_ws_tickets')->insert([
                'user_id'    => (int) $user->id,
                'ticket'     => $ticket,
                'expires_at' => $expiresAt,
                'created_at' => now(),
            ...
            // Fallback: gebruik Laravel cache (60 seconden TTL)
            \Illuminate\Support\Facades\Cache::put(
                "ws_ticket:{$ticket}",
                ['user_id' => (int) $user->id, 'role' => $user->role ?? 'client'],
                60
            );
        }
    ...
    /**
     * GET broadcasting/config
     * Returns Pusher/Soketi configuration for client-side initialization
     */
    public function broadcastingConfig(Request $request): JsonResponse
    {
        ...
        if (!Schema::hasTable('gymies_users')) {
            return response()->json(['message' => 'Database not initialized'], 500);
        }

        $broadcasterType = strtolower(trim((string) config('broadcasting.default', 'pusher')));
        $config = [
            'broadcaster' => $broadcasterType,
            'key' => config("broadcasting.connections.{$broadcasterType}.key"),
            'cluster' => config("broadcasting.connections.{$broadcasterType}.options.cluster"),
            'host' => config("broadcasting.connections.{$broadcasterType}.host"),
            'port' => config("broadcasting.connections.{$broadcasterType}.port"),
            'scheme' => config("broadcasting.connections.{$broadcasterType}.scheme", 'https'),
     ...
     * Authenticates private and presence channels for the user
     */
    public function broadcastingAuth(Request $request): JsonResponse
    {
        ...
        // Validate channel access based on naming conventions
        // private-* channels: user must own the channel (private-user-{id})
        // presence-* channels: user must be in the group (presence-gym-{id}, etc.)
        if (strpos($channelName, 'private-user-') === 0) {
            $channeledUserId = (int) substr($channelName, 13);
            if ((int) $user->id !== $channeledUserId) {
                return response()->json(['message' => 'Unauthorized channel access'], 403);
            }
        } elseif (strpos($channelName, 'presence-') === 0) {
            // Presence channels allowed for all authenticated users
            // Fine-grained access control depends on channel purpose (gym membership, etc.)
        } else {
            return response()->json(['message' => 'Invalid channel name'], 400);
        }

        // Use Laravel Broadcasting's authenticator
        $auth = auth()->guard('api');
        $authorizationKey = hash_hmac(
            'sha256',
        ...
        return response()->json([
            'auth' => config("broadcasting.connections.pusher.key") . ':' . $authorizationKey,
