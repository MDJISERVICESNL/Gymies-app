...
final class GymiesBookingController extends Controller
{
    ...
    private function doIndex(Request $request): JsonResponse
    {
        ...
        if ($this->columnExists('gymies_bookings', 'proposed_scheduled_at')) {
            $select[] = 'b.proposed_scheduled_at';
            $select[] = 'b.proposed_duration_minutes';
        }
        if ($this->columnExists('gymies_bookings', 'proposed_by_user_id')) {
            $select[] = 'b.proposed_by_user_id';
        }
        if ($this->columnExists('gymies_bookings', 'proposed_at')) {
            $select[] = 'b.proposed_at';
        }
        $hasPackageId = $this->columnExists('gymies_bookings', 'package_id');
        if ($hasPackageId) {
            $select[] = 'b.package_id';
            $select[] = 'b.sessions_remaining';
        }
        if ($this->columnExists('gymies_bookings', 'check_in_at')) {
            $select[] = 'b.check_in_at';
        }
        if ($this->columnExists('gymies_bookings', 'payment_method')) {
            $select[] = 'b.payment_method';
        }
        if ($this->columnExists('gymies_bookings', 'spoed_inval_original_trainer_id')) {
            $select[] = 'b.spoed_inval_original_trainer_id';
        }
        if ($this->columnExists('gymies_bookings', 'gym_location_id')) {
        ...
        if ($hasPackageId && $this->tableExists('gymies_packages')) {
            $query->leftJoin('gymies_packages as pkg', 'b.package_id', '=', 'pkg.id');
            $select[] = 'pkg.name as package_name';
        }
        if ($this->columnExists('gymies_bookings', 'gym_location_id') && $this->tableExists('gym_locations')) {
            $query->leftJoin('gym_locations as gl', 'b.gym_location_id', '=', 'gl.id');
        ...
        // Expire verlopen verplaatsingsverzoeken (24u deadline) — alleen als nodig
        $needsRefresh = false;
        foreach ($rows as $r) {
        ...
        // Alleen opnieuw ophalen als er daadwerkelijk iets gewijzigd is
        if ($needsRefresh) {
            $rows = $query->orderBy('b.scheduled_at', 'desc')->get();
        }

        $reviewedBookingIds = [];
        if ($this->tableExists('gymies_reviews')) {
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        if ($packageId !== null && $this->tableExists('gymies_packages') && $this->columnExists('gymies_bookings', 'package_id')) {
        ...
        if ($packageId !== null && $this->columnExists('gymies_bookings', 'package_id')) {
            $insertPayload['package_id'] = $packageId;
        }
        if ($sessionsRemaining !== null && $this->columnExists('gymies_bookings', 'sessions_remaining')) {
            $insertPayload['sessions_remaining'] = $sessionsRemaining;
        }
        if ($this->columnExists('gymies_bookings', 'gym_location_id') && $request->filled('gym_location_id')) {
            $gymLocationId = (int) $request->input('gym_location_id');
            if ($organisationId && $this->tableExists('gym_locations')) {
                $loc = DB::table('gym_locations')->where('id', $gymLocationId)->where('organisation_id', $organisationId)->first();
                if ($loc) {
                    $insertPayload['gym_location_id'] = $gymLocationId;
                }
            }
        }
        // Transaction: booking + notificaties atomisch opslaan
        DB::beginTransaction();
        try {
            // Double-entry check binnen transaction (met lock) om race condition te voorkomen
            ...
            // Notificaties voor trainer en klant
        ...
        if (property_exists($row, 'package_id') && $row->package_id !== null) {
            ...
            $pkg = $this->tableExists('gymies_packages') ? DB::table('gymies_packages')->where('id', $row->package_id)->value('name') : null;
    ...
    public function storeDirectBook(GymiesDirectBookingRequest $request): JsonResponse
    {
        ...
        if ($this->columnExists('gymies_trainer_profiles', 'women_only') && $this->columnExists('gymies_users', 'gender')) {
        ...
        if ($paymentMethod === 'cash' && $this->columnExists('gymies_trainer_profiles', 'accepts_cash')) {
    ...
    public function confirmCashPayment(Request $request, string $id): JsonResponse
    {
        ...
        if (!empty($booking->cash_confirmed_at)) {
        ...
        if ($this->columnExists('gymies_bookings', 'cash_confirmed_at')) {
    ...
    public function confirm(Request $request, string $id): JsonResponse
    {
        ...
        try {
            ...
            $amountCents = $request->input('amount_cents');
            if ($amountCents !== null && DB::getSchemaBuilder()->hasColumn('gymies_bookings', 'amount_cents')) {
                $updatePayload['amount_cents'] = (int) $amountCents;
            }
            if (DB::getSchemaBuilder()->hasColumn('gymies_bookings', 'location_type')) {
            ...
            if ($this->columnExists('gymies_bookings', 'gym_location_id') && $request->filled('gym_location_id')) {
                $gymLocationId = (int) $request->input('gym_location_id');
                $orgId = null;
                if ($this->columnExists('gymies_bookings', 'organisation_id')) {
                    $orgId = $booking->organisation_id ? (int) $booking->organisation_id : null;
                }
                if (!$orgId && $this->tableExists('gymies_organisation_trainers')) {
                    $orgLink = DB::table('gymies_organisation_trainers')
                        ->where('trainer_user_id', $trainerId)
                        ->where('status', 'active')
                        ->where('is_primary', 1)
                ...
                if ($orgId && $this->tableExists('gym_locations')) {
                    $loc = DB::table('gym_locations')->where('id', $gymLocationId)->where('organisation_id', $orgId)->first();
                    if ($loc) {
                        $updatePayload['gym_location_id'] = $gymLocationId;
                    }
                }
            }
            // Overlap check VOOR update — voorkomt race condition window
    ...
    public function rescheduleRespond(Request $request, string $id): JsonResponse
    {
        ...
        if (!$this->columnExists('gymies_bookings', 'proposed_scheduled_at') || empty($booking->proposed_scheduled_at)) {
        ...
        $proposedByUserId = $this->columnExists('gymies_bookings', 'proposed_by_user_id') && $booking->proposed_by_user_id !== null
        ...
        if ($isTrainer && $this->columnExists('gymies_bookings', 'gym_location_id') && $request->filled('gym_location_id')) {
            $gymLocationId = (int) $request->input('gym_location_id');
            $orgId = $booking->organisation_id ? (int) $booking->organisation_id : null;
            if (!$orgId && $this->tableExists('gymies_organisation_trainers')) {
                $orgLink = DB::table('gymies_organisation_trainers')
    ...
    {
        if (!$this->columnExists('gymies_bookings', 'proposed_at') || $booking->proposed_at === null) {
            return;
        }
    ...
    /**
     * Annuleringsbeleid: gebruikt trainer-specifieke regels uit gymies_cancellation_policies.
     * Fallback naar platformbeleid (48u/24u) als de trainer geen eigen regels heeft.
     *
     * @return array{refund_percent: int, refund_method_allowed: string, cancellation_policy_message: string, penalty_applies: bool, penalty_cents: int, grace_period: bool}
     */
    private function cancellationOutcome(float $hoursUntil, bool $isTrainer, int $trainerUserId, ?string $bookingCreatedAt = null): array
    {
        ...
        // Laad trainer-specifieke annuleringsregels (gesorteerd op hours_before DESC)
        $policies = collect();
        ...
        if ($isTrainer) {
            ...
            $penaltyCents = 0;
            // Penalty-drempel: kortste trainer-regel, of platform-default 24u
            $penaltyThresholdHours = $policies->isNotEmpty()
                ? (int) $policies->last()->hours_before   // laagste hours_before
                : self::CANCELLATION_HOURS_CREDITS_ONLY;
            if ($hoursUntil < $penaltyThresholdHours) {
                $penaltyApplies = true;
                $penaltyCents = self::TRAINER_PENALTY_CENTS;
            }
            // Methode: hoogste trainer-regel bepaalt bank-keuze grens
            $fullChoiceThreshold = $policies->isNotEmpty()
                ? (int) $policies->first()->hours_before  // hoogste hours_before
                : self::CANCELLATION_HOURS_FULL_CHOICE;
            return [
                'refund_percent' => 100,
                'refund_method_allowed' => $hoursUntil >= $fullChoiceThreshold ? 'bank_or_wallet' : 'wallet_only',
                'cancellation_policy_message' => $penaltyApplies
                    ? "Trainer annuleert binnen {$penaltyThresholdHours} uur. Klant krijgt 100% terug. Trainer krijgt een boete van €25."
        ...
        // ── Klant annuleert: trainer-regels ──
        if ($policies->isNotEmpty()) {
            foreach ($policies as $i => $policy) {
                if ($hoursUntil >= (int) $policy->hours_before) {
                    $isTopTier = ($i === 0); // eerste = hoogste hours_before
                    $refundPercent = (int) $policy->refund_percent;
                    $methodAllowed = $isTopTier && $refundPercent > 0 ? 'bank_or_wallet' : ($refundPercent > 0 ? 'wallet_only' : 'none');
                    return [
                        'refund_percent' => $refundPercent,
                        'refund_method_allowed' => $methodAllowed,
                        'cancellation_policy_message' => $this->formatCancellationPolicyMessage(
                            (int) $policy->hours_before,
                            $refundPercent,
                            $policy->name ?? null
                        ),
                        'penalty_applies' => false,
            ...
            // Geen tier matcht: binnen de kortste termijn → geen restitutie
            $shortestHours = (int) $policies->last()->hours_before;
            return [
                'refund_percent' => 0,
                'refund_method_allowed' => 'none',
                'cancellation_policy_message' => "Binnen {$shortestHours} uur voor aanvang is geen restitutie mogelijk volgens het beleid van deze trainer.",
                'penalty_applies' => false,
                'penalty_cents' => 0,
                'grace_period' => false,
            ];
        ...
        // ── Fallback: platformbeleid (48u / 24u) ──
    ...
    /**
     * Preview: welke restitutie geldt bij annuleren (trainer-regels, fallback platformbeleid).
     */
    public function cancellationPreview(Request $request, string $id): JsonResponse
    {
        ...
        $outcome = $this->cancellationOutcome($hoursUntil, $isTrainer, (int) $booking->trainer_user_id, $booking->created_at ?? null);
    ...
    /**
     * Annuleren: trainer-regels bepalen restitutie. Fallback platformbeleid 48u/24u.
     */
    public function cancel(Request $request, string $id): JsonResponse
    {
        ...
        try {
            ...
            if ($refundPercent > 0 && $amountCents > 0) {
                if ($refundMethod === 'wallet' && $this->tableExists('gymies_wallet_transactions') && $this->columnExists('gymies_users', 'wallet_balance_cents')) {
                    // Atomische increment voorkomt race condition bij gelijktijdige annuleringen
                ...
                } elseif ($refundMethod === 'bank' && $this->tableExists('gymies_admin_refunds')) {
                    $bankFeeCents = 0;
                    if (!$isGracePeriod) {
                        $bankFeeCents = 99;
                        if ($this->tableExists('gymies_system_settings')) {
    ...
    {
        if (!$this->columnExists('gymies_users', 'trainer_balance_cents')) {
            return;
        }
        $current = (int) (DB::table('gymies_users')->where('id', $trainerUserId)->value('trainer_balance_cents') ?? 0);
        $newBalance = $current - self::TRAINER_PENALTY_CENTS;
        DB::table('gymies_users')->where('id', $trainerUserId)->update(['trainer_balance_cents' => $newBalance, 'updated_at' => now()]);
        if ($this->tableExists('gymies_trainer_penalties')) {
            DB::table('gymies_trainer_penalties')->insert([
                'trainer_user_id' => $trainerUserId,
                'amount_cents' => -self::TRAINER_PENALTY_CENTS,
        ...
        if ($this->columnExists('gymies_trainer_profiles', 'consecutive_completed')) {
    ...
    /**
     * Klant: factuuraanvraag indienen voor boeking (creëert support ticket).
     */
    public function invoiceRequest(Request $request, string $id): JsonResponse
    {
        $user = $request->attributes->get('gymies_user');
        if (!$user) {
            return response()->json(['message' => 'Unauthorized'], 401);
        ...
        if ((int) $booking->client_user_id !== (int) $user->id) {
            return response()->json(['message' => 'Je kunt alleen voor je eigen boekingen een factuur aanvragen.'], 403);
        }
        // Create a support ticket of type invoice_request
        if (!$this->tableExists('gymies_support_tickets')) {
            return response()->json(['message' => 'Support systeem niet beschikbaar.'], 500);
        }
        $ticketData = [
            'user_id' => (int) $user->id,
            'type' => 'invoice_request',
            'subject' => 'Factuuraanvraag voor boeking #' . $id,
            'status' => 'open',
            'booking_id' => (int) $id,
            'created_at' => now(),
            'updated_at' => now(),
        ];
        if ($this->columnExists('gymies_support_tickets', 'is_dispute')) {
            $ticketData['is_dispute'] = false;
        }
        $ticketId = DB::table('gymies_support_tickets')->insertGetId($ticketData);
        $message = $request->input('message', 'Ik wil graag een factuur ontvangen voor deze boeking.');
        if ($this->tableExists('gymies_support_messages')) {
            DB::table('gymies_support_messages')->insert([
                'ticket_id' => $ticketId,
                'user_id' => (int) $user->id,
                'body' => $message,
                'created_at' => now(),
        ...
        return response()->json(['data' => ['ticket_id' => $ticketId, 'status' => 'open'], 'message' => 'Factuuraanvraag ingediend.'], 201);
    }
    ...
    public function storeReview(Request $request, string $id): JsonResponse
    {
        ...
        if ((int) $booking->client_user_id !== (int) $user->id) {
            return response()->json(['message' => 'Je kunt alleen je eigen sessies beoordelen.'], 403);
        }
    ...
    public function reportTrainerNoShow(Request $request, string $id): JsonResponse
    {
        ...
        try {
            ...
            if ($amountCents > 0 && $this->tableExists('gymies_wallet_transactions') && $this->columnExists('gymies_users', 'wallet_balance_cents')) {
    ...
    public function getBookingReview(Request $request, string $id): JsonResponse
    {
        ...
        $responseText = null;
        if ($this->tableExists('gymies_review_responses')) {
    ...
    {
        if (!$this->tableExists('gymies_trainer_profiles') || !$this->columnExists('gymies_trainer_profiles', 'lead_time_minutes')) {
    ...
    private function getTrainerMaxBookingDaysAhead(int $trainerId): int
    {
        if (!$this->tableExists('gymies_trainer_profiles')
            || !$this->columnExists('gymies_trainer_profiles', 'booking_max_days_ahead')) {
     ...
     */
    private function hasPendingConfirmedOrReservedOverlap(int $trainerId, string $scheduledAt, int $duration, int $excludeBookingId = 0): bool
    {
        $query = DB::table('gymies_bookings')
            ->where('trainer_user_id', $trainerId)
            ->whereRaw("? < DATE_ADD(scheduled_at, INTERVAL duration_minutes MINUTE) AND DATE_ADD(?, INTERVAL ? MINUTE) > scheduled_at", [
        ...
        $hasReservedCol = $this->columnExists('gymies_bookings', 'reserved_until');
        if ($hasReservedCol) {
    ...
    public function submitGhostRating(Request $request, string $id): JsonResponse
    {
        ...
        if (!$this->tableExists('gymies_ghost_ratings')) {
    ...
    private function injectRescheduleCardMessage(
        ...
        if (!$this->tableExists('gymies_conversations') || !$this->tableExists('gymies_messages')) {
            return;
        }
    ...
    {
        if (!$this->tableExists('gymies_messages') || !$this->tableExists('gymies_conversations')) {
            return;
        }
    ...
    // ──────────────────────────────────────────────────────────────
    // Slot Holds — tijdelijke reservering om race-conditions te voorkomen
    // ──────────────────────────────────────────────────────────────

    /** Hoe lang een hold geldig is (minuten). */
    private const HOLD_EXPIRY_MINUTES = 5;

    /**
     * POST bookings/hold-slot
     *
     * Maak een tijdelijke hold aan zodat geen andere klant hetzelfde slot kan boeken.
     * De hold verloopt automatisch na 5 minuten.
     */
    public function holdSlot(Request $request): JsonResponse
    {
        ...
        $trainerId    = $request->input('trainer_user_id') ?? $request->input('trainer_id');
        $scheduledAt  = $request->input('scheduled_at');
        $slotId       = $request->input('availability_slot_id') ?? $request->input('slot_id');

        if (!$trainerId || !$scheduledAt) {
            return response()->json(['message' => 'trainer_user_id en scheduled_at zijn verplicht.'], 422);
        }

        // Self-heal: maak tabel aan als deze niet bestaat
        if (!Schema::hasTable('gymies_slot_holds')) {
            Schema::create('gymies_slot_holds', function (\Illuminate\Database\Schema\Blueprint $table) {
                $table->bigIncrements('id');
                $table->unsignedBigInteger('user_id');
                $table->unsignedBigInteger('trainer_user_id');
                $table->string('availability_slot_id')->nullable();
                $table->dateTime('scheduled_at');
                $table->string('hold_token', 64)->unique();
                $table->dateTime('expires_at');
                $table->dateTime('released_at')->nullable();
                $table->timestamps();
                $table->index(['trainer_user_id', 'scheduled_at', 'expires_at']);
            });
        }

        // Verwijder verlopen holds
        DB::table('gymies_slot_holds')
            ->where('expires_at', '<', now())
            ->whereNull('released_at')
            ->delete();

        // Check of dit slot al een actieve hold heeft van iemand anders
        $existingHold = DB::table('gymies_slot_holds')
            ->where('trainer_user_id', (int) $trainerId)
            ->where('scheduled_at', Carbon::parse($scheduledAt))
            ->where('expires_at', '>', now())
            ->whereNull('released_at')
            ->first();

        if ($existingHold && (int) $existingHold->user_id !== (int) $user->id) {
            return response()->json([
                'message' => 'Dit tijdslot is momenteel gereserveerd door een andere klant. Probeer het over een paar minuten opnieuw.',
                'error' => 'slot_held',
            ], 409);
        }

        // Als de huidige user al een hold heeft, verleng die
        if ($existingHold && (int) $existingHold->user_id === (int) $user->id) {
            DB::table('gymies_slot_holds')
                ->where('id', $existingHold->id)
                ->update(['expires_at' => now()->addMinutes(self::HOLD_EXPIRY_MINUTES), 'updated_at' => now()]);

            return response()->json([
                'hold' => [
                    'id' => (string) $existingHold->id,
                    'hold_token' => $existingHold->hold_token,
                    'expires_at' => now()->addMinutes(self::HOLD_EXPIRY_MINUTES)->toIso8601String(),
                ],
            ]);
        }

        // Nieuwe hold aanmaken
        $holdToken = bin2hex(random_bytes(16));
        $expiresAt = now()->addMinutes(self::HOLD_EXPIRY_MINUTES);

        $holdId = DB::table('gymies_slot_holds')->insertGetId([
            'user_id'              => (int) $user->id,
            'trainer_user_id'      => (int) $trainerId,
    ...
    /**
     * DELETE bookings/hold-slot/{holdId}
     *
     * Geef een slot-hold vrij (bijv. als de klant wegnavigeerd zonder te boeken).
     */
    public function releaseSlot(Request $request, string $holdId): JsonResponse
    {
        $user = $request->attributes->get('gymies_user');
        if (!$user) {
            return response()->json(['message' => 'Unauthorized'], 401);
        ...
        $hold = DB::table('gymies_slot_holds')->where('id', $holdId)->first();

        if (!$hold) {
            return response()->json(['ok' => true]); // Idempotent: al verwijderd of nooit bestaan
        }

        if ((int) $hold->user_id !== (int) $user->id) {
            return response()->json(['message' => 'Niet jouw hold.'], 403);
        }
    ...
    // ──────────────────────────────────────────────────────────────
    // Trainer Reject — expliciete afwijzing van boeking
    // ──────────────────────────────────────────────────────────────

    /**
     * POST trainer/bookings/{id}/reject
     *
     * Trainer wijst een boeking af. Intern vertaald naar cancellation
     * met reason 'trainer_rejected' zodat de juiste refund-logica actief is.
     */
    public function reject(Request $request, string $id): JsonResponse
    {
        ...
        if ((int) $booking->trainer_user_id !== (int) $user->id) {
            return response()->json(['message' => 'Je kunt alleen je eigen boekingen afwijzen.'], 403);
        }

        if (!in_array($booking->status, ['pending', 'confirmed'], true)) {
            return response()->json(['message' => 'Deze boeking kan niet meer afgewezen worden.'], 422);
        }
        ...
        try {
            DB::table('gymies_bookings')
                ...
                ->update([
                    ...
                    'cancelled_at'        => now(),
                    'cancelled_by_user_id' => $user->id,
                    'cancellation_reason' => 'trainer_rejected',
                    'updated_at'          => now(),
                ]);

            // Volledige refund naar wallet als er betaald is
            $amountCents = (int) ($booking->amount_cents ?? 0);
            $clientUserId = (int) $booking->client_user_id;

            if ($amountCents > 0
                && $this->tableExists('gymies_wallet_transactions')
                && $this->columnExists('gymies_users', 'wallet_balance_cents')
            ) {
                DB::table('gymies_users')
                    ->where('id', $clientUserId)
                    ->increment('wallet_balance_cents', $amountCents);

                $newBalance = (int) (DB::table('gymies_users')
                    ->where('id', $clientUserId)
                    ->value('wallet_balance_cents') ?? 0);
                ...
                $walletInsert = [
                    ...
                    'balance_after_cents' => $newBalance,
                    'booking_id'         => (int) $booking->id,
                    'reason'             => 'Boeking afgewezen door trainer. Volledige restitutie.',
                    'reference_type'     => 'rejection_refund',
                    'admin_user_id'      => null,
                    'created_at'         => now(),
                ];
                if ($this->columnExists('gymies_wallet_transactions', 'expires_at')) {
            ...
            if ($this->tableExists('gymies_notifications')) {
                DB::table('gymies_notifications')->insert([
                    'user_id'    => $clientUserId,
                    'type'       => 'booking_rejected',
                    'title'      => 'Boeking afgewezen',
                    'body'       => 'Je boeking is helaas afgewezen door de trainer.' . ($amountCents > 0 ? ' Het bedrag is teruggestort naar je wallet.' : ''),
                    'data'       => json_encode(['booking_id' => (int) $booking->id]),
                    'read_at'    => null,
                    'created_at' => now(),
                ]);
            }
            ...
            DB::rollBack();
            report($e);
            return response()->json(['message' => 'Er ging iets mis bij het afwijzen.'], 500);
        }

        return response()->json([
            'ok'      => true,
            'message' => 'Boeking afgewezen.',
        ]);
    }
}
