...
final class GymiesGymInviteController extends GymiesGymController
{
    public function index(Request $request): JsonResponse
    {
        ...
        if (!Schema::hasTable('gym_invites')) {
            return response()->json(['data' => []]);
        }

        $orgId = (int) $ctx['organisation_id'];
        $rows = DB::table('gym_invites')
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        $token = Str::random(48);
        while (DB::table('gym_invites')->where('token', $token)->exists()) {
            $token = Str::random(48);
        }
        ...
        $id = DB::table('gym_invites')->insertGetId([
        ...
        $row = DB::table('gym_invites')->where('id', $id)->first();
    ...
    public function accept(Request $request): JsonResponse
    {
        ...
        if (!Schema::hasTable('gym_invites') || !Schema::hasTable('gymies_organisation_trainers')) {
            return response()->json(['message' => 'Uitnodigingen niet beschikbaar.'], 422);
        }
        ...
        DB::table('gym_invites')->where('id', $row->id)->increment('used_count');
