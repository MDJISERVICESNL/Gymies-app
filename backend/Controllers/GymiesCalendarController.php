...
class GymiesCalendarController extends Controller
{
    ...
    public function feed(string $token): Response|JsonResponse
    {
        ...
        $selectCols = ["id", "display_name", "email", "role"];
        if (Schema::hasColumn("gymies_users", "calendar_token_created_at")) {
            $selectCols[] = "calendar_token_created_at";
        }
        ...
        // Check token expiry: 90-day TTL if calendar_token_created_at exists
        if (Schema::hasColumn("gymies_users", "calendar_token_created_at") && $user->calendar_token_created_at) {
            $tokenCreatedAt = \Carbon\Carbon::parse($user->calendar_token_created_at);
            if ($tokenCreatedAt->addDays(90)->isPast()) {
                return response()->json(["message" => "Token verlopen. Genereer een nieuw token."], 403);
            }
        }
