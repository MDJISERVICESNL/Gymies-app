...
final class GymiesAmbassadorController extends Controller
{
    ...
    /**
     * POST /api/gymies/admin/ambassador/{id}/verify-iban
     * Admin verifieert IBAN na handmatige controle (bijv. proefstorting).
     */
    public function adminVerifyIban(Request $request, string $id): JsonResponse
    {
        $this->requireAdmin($request);
        GymiesSchemaEnsure::ambassadorTables();

        $amb = DB::table('gymies_ambassadors')->where('id', (int) $id)->first(['id', 'iban', 'iban_verified_at']);
        if (!$amb) {
            return response()->json(['message' => 'Niet gevonden.'], 404);
        }
        if (empty($amb->iban)) {
            return response()->json(['message' => 'Geen IBAN ingesteld door ambassadeur.'], 422);
        }
        if ($amb->iban_verified_at !== null) {
            return response()->json(['message' => 'IBAN is al geverifieerd.', 'iban_verified_at' => $amb->iban_verified_at], 200);
        }
        ...
        return response()->json([
            'ok' => true,
            'message' => 'IBAN geverifieerd. Uitbetaling is nu mogelijk.',
            'iban_verified_at' => now()->toIso8601String(),
        ]);
    }
     ...
     * Aanroepen vanuit GymiesPaymentController na succesvolle Mollie-betaling.
     *
     * $trainerUserId: de trainer bij wie de sporter boekt. Als de ambassadeur zelf
     * deze trainer is, wordt de conversie alsnog toegestaan mits de sporter via de
     * code kwam (de sporter is immers een echte klant). Alleen als de sporter zelf
     * de ambassadeur is, wordt de conversie geblokkeerd.
     */
    ...
    private function codeExists(string $code): bool
    {
        $inPromo = Schema::hasTable('gymies_promo_codes')
            && DB::table('gymies_promo_codes')->where('code', $code)->lockForUpdate()->exists();

        $inAmbassadors = Schema::hasTable('gymies_ambassadors')
            && DB::table('gymies_ambassadors')->where('discount_code', $code)->lockForUpdate()->exists();

        $inReferrals = Schema::hasTable('gymies_referrals')
            && DB::table('gymies_referrals')->where('referral_code', $code)->lockForUpdate()->exists();
