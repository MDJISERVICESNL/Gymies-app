...
final class GymiesOpsController extends Controller
{
    ...
    public function runBackup(Request $request): JsonResponse
    {
        ...
        if ($exitCode !== 0) {
            return response()->json(['message' => 'Backup kon niet gestart worden.'], 500);
        }
    ...
    public function runWebSync(Request $request): JsonResponse
    {
        ...
        $command = sprintf(
            "nohup bash -lc %s >> %s 2>&1 &",
            escapeshellarg(escapeshellcmd($scriptPath)),
            escapeshellarg($logPath)
        );

        @exec($command, $output, $exitCode);
        if ($exitCode !== 0) {
            return response()->json(['message' => 'Web sync kon niet gestart worden.'], 500);
        }
